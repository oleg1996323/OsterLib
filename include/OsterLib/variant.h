            ...
            auto try_emplace = [&]<size_t ID>()
            {
                ...
                if constexpr (sizeof...(ARGS)>0 && std::is_constructible_v<std::variant_alternative_t<ID, VariantType>,ARGS...>){
                  ...
                  if constexpr(sizeof...(ARGS)>0)
                        static_assert(std::is_constructible_v<std::variant_alternative_t<ID, VariantType>,ARGS...>,
                              "variant must be constructible by arguments");
                  else static_assert(std::is_default_constructible_v<std::variant_alternative_t<ID, VariantType>>,
                              "variant must be constructible at least by default");
                }
            };
