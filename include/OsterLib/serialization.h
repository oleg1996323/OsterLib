...
namespace serialization{
    ...
    struct Serialize<NETWORK_ORDER,T[SZ]>{
        SerializationEC operator()(const T (&val)[SZ],std::vector<char>& buf) const noexcept{
            if constexpr(sizeof(T)==1){
    ...
    template<bool NETWORK_ORDER,typename T,size_t SZ>
    struct Deserialize<NETWORK_ORDER,T[SZ]>{
        /// @brief Deserialize data from buffer to specified type
        /// @tparam T Supported types: integral, floating-point, enum (including scoped enum)
        /// @tparam NETWORK_ORDER If true, converts from network (big-endian) byte order
        /// @param buf Input data buffer (read-only)
        ...
        /// @note Supports both runtime and constexpr contexts
        /// @warning Buffer must be properly aligned for type T
        SerializationEC operator()(T (&to_deserialize)[SZ],StreamSerializer& buf) const noexcept{
            size_t remain = SZ;
    ...
    struct Serial_size<T[SZ]>{
        size_t operator()(const T (&val)[SZ]) const noexcept{
            size_t result = 0;
            if constexpr (min_serial_size<T>()==max_serial_size<T>())
    ...
    struct Serialize<NETWORK_ORDER,T>{
        SerializationEC operator()(const T& range,std::vector<char>& buf) const noexcept{
            ...
            SerializationEC err;
            if constexpr(std::is_same_v<T,std::forward_list<std::ranges::range_value_t<T>>>)
                err = serialize<NETWORK_ORDER>(std::distance(range.begin(),range.end()),buf);
            else err = serialize<NETWORK_ORDER>(std::size(range),buf);
    ...
    struct Deserialize<NETWORK_ORDER,T>{
        SerializationEC operator()(T& to_deserialize,StreamSerializer& buf) const noexcept{
            ...
            size_t sz = 0;
            if constexpr(std::is_same_v<T,std::forward_list<
                std::ranges::range_value_t<T>>>)
            {
                if(buf.registered_size()==1){
                    buf.register_value(std::distance(
                            to_deserialize.begin(),
                            to_deserialize.end()));
                }
                else buf.get_value_at(sz,1);
            }
            else sz = std::size(to_deserialize);
            for (size_t i = 0; i < range_sz-sz; ++i){
                if constexpr(is_associative_container_v<T>){
                    ...
                    if constexpr(std::is_same_v<T,std::forward_list<
                            std::ranges::range_value_t<T>>>){
                        to_deserialize.emplace_front(std::move(item));
                        buf.update_value_at(sz+1,1);
                    }
                    else
