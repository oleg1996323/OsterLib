...
class ConnectionIO{
    ...
    std::int64_t __send_internal__(std::error_code& err) noexcept{
        if(!socket_.valid()){
    ...
    template<typename... ARGS>
    std::int64_t send(std::error_code& err,
            std::function<void(const std::vector<char>&)>&& after_serialization,
            ARGS&&... values) noexcept
    {
        if(serialize(std::forward<ARGS>(values)...)!=serialization::SerializationEC::NONE){
            ...
            after_serialization(send_buffer_.data().back());
            return __send_internal__(err);
        }
    }
    template<typename... ARGS>
    std::int64_t receive(std::error_code& err,
            ARGS&... values) noexcept
    {
        assert(recv_buffer_.capacity()>0);        
        auto recv_res = __recv_internal__(err);
        if(recv_res==-1) return -1;
