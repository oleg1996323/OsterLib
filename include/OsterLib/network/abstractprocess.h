...
class AbstractTaskHandler{
    ...
    template<typename TYPE,typename std::enable_if<std::is_copy_assignable_v<TYPE>>::type>
    std::optional<TYPE> get_as(std::error_code& err) noexcept;
    template<typename TYPE>
    const TYPE* get_as_ptr(std::error_code& err) noexcept;
...
class TypedTaskHandler<TaskMode::Sync,Result>:public AbstractTaskHandler{
    ...
    result_return_t get_result(std::error_code& err) noexcept{
        if constexpr(!std::is_same_v<void,result_return_t>){
            ...
            status = result_.wait_for(std::chrono::seconds(0));
    ...
    const result_type* get_pointer_result(std::error_code& err) noexcept{
        if(is_ready(err))
            return &result_.get();
        else return nullptr;
    }
...
class TypedTaskHandler<TaskMode::Thread,Result>:public AbstractTaskHandler{
    ...
    virtual result_return_t get_result(std::error_code& err) noexcept{
        if constexpr(!std::is_same_v<void,result_return_t>){
            ...
            auto status = result_.wait_for(std::chrono::seconds(0));
            if (status == std::future_status::ready){
                try {
                    err.clear();
                    return;
...
template<typename TYPE>
const TYPE* AbstractTaskHandler::get_as_ptr(std::error_code& err) noexcept{
    switch(mode_){
        case TaskMode::Sync:{
            if(TypedTaskHandler<TaskMode::Sync,TYPE>* casted = 
                dynamic_cast<TypedTaskHandler<TaskMode::Sync,TYPE>*>(this);
                casted!=nullptr)
                return casted->get_pointer_result(err);
            else nullptr;
        }
        break;
        case TaskMode::Thread:{
