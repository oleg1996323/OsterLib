...
TEST(Client_server,ping){
    ...
    for(int i = 0;i<100;++i){
        ...
        ASSERT_EQ(ServerPingProcess::count_recv.load(std::memory_order::relaxed),5);
        ASSERT_EQ(ServerPingProcess::count_sent.load(std::memory_order::relaxed),5);
        ASSERT_EQ(ClientPingProcess::count_recv.load(std::memory_order::relaxed),5);
        ASSERT_EQ(ClientPingProcess::count_sent.load(std::memory_order::relaxed),5);
...
//                     std::monostate());
//             cmd->wait_ready(10);
//         ASSERT_EQ(ServerProcess::received,"Hello server");
//         ASSERT_EQ(cmd->received()->data_frame(),"Hello client");
// }
